// synchronous adaptation of:
// https://github.com/substack/html-tokenize
...
Tokenize.prototype.parse = function (str) {
  if (typeof str === 'string') str = Buffer.from(str)
  this._parse(str)
  return this._result
}

Tokenize.prototype._parse = function (buf) {
  var i = 0
  var offset = 0
  ...
  for (; i < buf.length; i++) {
    ...
    if (this.raw) {
      ...
      if (parts) {
        this._last.push([ 'text', parts[0] ])

        if (this.raw === strings.endComment ||
                this.raw === strings.endCdata) {
          this.state = 'text'
          this.buffers = []
          this._last.push([ 'close', parts[1] ])
        } else {
          this.state = 'open'
...
Tokenize.prototype._getTag = function () {
  ...
  for (var j = 0; j < this.buffers.length; j++) {
    ...
    for (var k = 0; k < buf.length; k++) {
      ...
      if (/[^\w-!\[\]]/.test(c)) { // eslint-disable-line no-useless-escape
